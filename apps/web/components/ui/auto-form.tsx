...
function AutoFormObject<SchemaType extends z.ZodObject<any, any>>({
  ...
  const [collapsiableValues, setCollapsiableValues] = useState<string[]>(
    Object.keys(shape).map((name) => name)
  );
  return (
    <Accordion
      type="multiple"
      className="space-y-5"
      value={collapsiableValues}
      onValueChange={setCollapsiableValues}
    >
      {Object.keys(shape).map((name) => {
        ...
        // if zod type is literal, we can skip it
        ...
        return (
          <FormField
            ...
            render={({ field }) => {
              ...
              // const get zod type description as placeholder
              // if (!fieldConfigItem.inputProps?.placeholder) {
              //   zodInputProps.placeholder = item._def.description;
              // }
...
function AutoForm<SchemaType extends ZodObjectOrWrapped>({
  ...
  onErrors?: (errors: z.ZodError<Record<string, any>>) => void;
}) {
